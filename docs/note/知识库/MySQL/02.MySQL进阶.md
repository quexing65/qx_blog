...
> [!note] 版本与使用说明
> 主体课件基于 MySQL 8.0.26。优化结论应结合真实数据分布、统计信息与 `EXPLAIN`/`EXPLAIN ANALYZE` 验证，不要把示例中的执行计划当作所有环境的固定结果。

## 1. 存储引擎

### 1.1 MySQL体系结构

![MySQL 进阶：MySQL体系结构](assets/进阶-MySQL体系结构-01.png)
...
![MySQL 进阶：存储引擎介绍](assets/进阶-存储引擎介绍-01.png)
...
![MySQL 进阶：存储引擎介绍](assets/进阶-存储引擎介绍-02.png)
...
![MySQL 进阶：存储引擎介绍](assets/进阶-存储引擎介绍-03.png)
...
![MySQL 进阶：InnoDB](assets/进阶-InnoDB-01.png)
...
![MySQL 进阶：InnoDB](assets/进阶-InnoDB-02.png)
...
![MySQL 进阶：InnoDB](assets/进阶-InnoDB-03.png)

4). 逻辑存储结构

![MySQL 进阶：InnoDB](assets/进阶-InnoDB-04.png)
...
![MySQL 进阶：MyISAM](assets/进阶-MyISAM-01.png)
...
![MySQL 进阶：介绍](assets/进阶-介绍-01.png)
...
![MySQL 进阶：演示](assets/进阶-演示-01.png)

假如我们要执行的SQL语句为： select * from user where age = 45;

1). 无索引情况

![MySQL 进阶：演示](assets/进阶-演示-02.png)
...
![MySQL 进阶：演示](assets/进阶-演示-03.png)
...
| Hash索引 | 底层数据结构是用哈希表实现的,支持范围查询 |   | 只有精确匹配索引列的查询才有效, 不 |   |
...
![MySQL 进阶：二叉树](assets/进阶-二叉树-01.png)

如果主键是顺序插入的，则会形成一个单向链表，结构如下：

![MySQL 进阶：二叉树](assets/进阶-二叉树-02.png)
...
![MySQL 进阶：二叉树](assets/进阶-二叉树-03.png)
...
![MySQL 进阶：B-Tree](assets/进阶-B-Tree-01.png)

知识小贴士: 树的度数指的是一个节点的子节点个数。

我们可以通过一个数据结构可视化的网站来简单演示一下。 https://www.cs.usfca.edu/~gall es/visualization/BTree.html

![MySQL 进阶：B-Tree](assets/进阶-B-Tree-02.png)

插入一组数据： 100 65 169 368 900 556 780 35 215 1200 234 888 158 90 1000 88 120 268 250 。然后观察一些数据插入过程中，节点的变化情况。

![MySQL 进阶：B-Tree](assets/进阶-B-Tree-03.png)
...
![MySQL 进阶：B+Tree](assets/进阶-B+Tree-04.png)
...
![MySQL 进阶：Hash](assets/进阶-Hash-01.png)

如果两个(或多个)键值，映射到一个相同的槽位上，他们就产生了hash冲突（也称为hash碰撞），可以通过链表来解决。

![MySQL 进阶：Hash](assets/进阶-Hash-02.png)
...
| 2.3.2 | 聚集索引&二级索引 |   |   |   |   |
| 而在在InnoDB | 存储引擎中，根据索引的存储形式，又可以分为以下两种： |   |   |   |   |
| 分类 |   | 含义 |   | 特点 |   |
| 聚集索引 | (Clustered | 将数据存储与索引放到了一块，索引结构的叶子 |   | 必须有 | ,而且只 |
| Index) |   | 节点保存了行数据 |   | 有一个 |   |
| 二级索引Index) | (Secondary | 将数据与索引分开存储，索引结构的叶子节点关联的是对应的主键 |   | 可以存在多个 |   |
| 聚集索引选取规则: |   |   |   |   |   |
| 如果存在主键，主键索引就是聚集索引。 |   |   |   |   |   |
...
![MySQL 进阶：索引分类](assets/进阶-索引分类-01.png)
...
![MySQL 进阶：索引分类](assets/进阶-索引分类-02.png)
...
![MySQL 进阶：索引分类](assets/进阶-索引分类-03.png)
...
![MySQL 进阶：索引语法](assets/进阶-索引语法-01.png)
...
![MySQL 进阶：索引语法](assets/进阶-索引语法-02.png)
...
![MySQL 进阶：SQL执行频率](assets/进阶-SQL执行频率-01.png)
...
![MySQL 进阶：SQL执行频率](assets/进阶-SQL执行频率-02.png)
...
![MySQL 进阶：慢查询日志](assets/进阶-慢查询日志-01.png)
...
![MySQL 进阶：慢查询日志](assets/进阶-慢查询日志-02.png)
...
![MySQL 进阶：慢查询日志](assets/进阶-慢查询日志-03.png)

B. 检查慢查询日志：

最终我们发现，在慢查询日志中，只会记录执行时间超多我们预设时间（2s）的SQL，执行较快的SQL是不会记录的。

![MySQL 进阶：慢查询日志](assets/进阶-慢查询日志-04.png)
...
![MySQL 进阶：profile详情](assets/进阶-profile详情-01.png)
...
![MySQL 进阶：profile详情](assets/进阶-profile详情-02.png)

查看指定SQL各个阶段的耗时情况:

![MySQL 进阶：profile详情](assets/进阶-profile详情-03.png)
...
![MySQL 进阶：explain](assets/进阶-explain-01.png)
...
| select_type | 或者子查询）、PRIMARY（主查询，即外层的查询）、UNION（UNION 中的第二个或者后面的查询语句）、 |
...
![MySQL 进阶：验证索引效率](assets/进阶-验证索引效率-01.png)
...
![MySQL 进阶：验证索引效率](assets/进阶-验证索引效率-02.png)
...
![MySQL 进阶：验证索引效率](assets/进阶-验证索引效率-03.png)
...
![MySQL 进阶：验证索引效率](assets/进阶-验证索引效率-04.png)
...
![MySQL 进阶：验证索引效率](assets/进阶-验证索引效率-05.png)
...
![MySQL 进阶：最左前缀法则](assets/进阶-最左前缀法则-01.png)
...
![MySQL 进阶：最左前缀法则](assets/进阶-最左前缀法则-02.png)

```sql
explain select * from tb_user where profession = '软件工程' and age = 31;
```

![MySQL 进阶：最左前缀法则](assets/进阶-最左前缀法则-03.png)

```sql
explain select * from tb_user where profession = '软件工程';
```

![MySQL 进阶：最左前缀法则](assets/进阶-最左前缀法则-04.png)
...
![MySQL 进阶：最左前缀法则](assets/进阶-最左前缀法则-05.png)

```sql
explain select * from tb_user where status = '0';
```

![MySQL 进阶：最左前缀法则](assets/进阶-最左前缀法则-06.png)
...
![MySQL 进阶：最左前缀法则](assets/进阶-最左前缀法则-07.png)
...
![MySQL 进阶：最左前缀法则](assets/进阶-最左前缀法则-08.png)
...
![MySQL 进阶：范围查询](assets/进阶-范围查询-01.png)
...
![MySQL 进阶：范围查询](assets/进阶-范围查询-02.png)
...
![MySQL 进阶：索引列运算](assets/进阶-索引列运算-01.png)
...
![MySQL 进阶：索引列运算](assets/进阶-索引列运算-02.png)
...
![MySQL 进阶：索引列运算](assets/进阶-索引列运算-03.png)
...
![MySQL 进阶：字符串不加引号](assets/进阶-字符串不加引号-01.png)
...
![MySQL 进阶：字符串不加引号](assets/进阶-字符串不加引号-02.png)
...
![MySQL 进阶：模糊查询](assets/进阶-模糊查询-01.png)
...
![MySQL 进阶：or连接条件](assets/进阶-or连接条件-01.png)
...
![MySQL 进阶：or连接条件](assets/进阶-or连接条件-02.png)
...
![MySQL 进阶：数据分布影响](assets/进阶-数据分布影响-01.png)

![MySQL 进阶：数据分布影响](assets/进阶-数据分布影响-02.png)
...
![MySQL 进阶：数据分布影响](assets/进阶-数据分布影响-03.png)

接下来，我们做一个操作将profession字段值全部更新为null。

然后，再次执行上述的两条SQL，查看SQL语句的执行计划。

![MySQL 进阶：数据分布影响](assets/进阶-数据分布影响-04.png)
...
![MySQL 进阶：SQL提示](assets/进阶-SQL提示-01.png)

索引情况如下:

![MySQL 进阶：SQL提示](assets/进阶-SQL提示-02.png)
...
![MySQL 进阶：SQL提示](assets/进阶-SQL提示-03.png)
...
![MySQL 进阶：SQL提示](assets/进阶-SQL提示-04.png)
...
![MySQL 进阶：SQL提示](assets/进阶-SQL提示-05.png)
...
![MySQL 进阶：SQL提示](assets/进阶-SQL提示-06.png)
...
![MySQL 进阶：SQL提示](assets/进阶-SQL提示-07.png)
...
![MySQL 进阶：覆盖索引](assets/进阶-覆盖索引-01.png)
...
![MySQL 进阶：覆盖索引](assets/进阶-覆盖索引-02.png)

id是主键，是一个聚集索引。 name字段建立了普通索引，是一个二级索引（辅助索引）。

B. 执行SQL : select * from tb_user where id = 2;

![MySQL 进阶：覆盖索引](assets/进阶-覆盖索引-03.png)

根据id查询，直接走聚集索引查询，一次索引扫描，直接返回数据，性能高。

C. 执行SQL：selet id,name from tb_user where name = 'Arm';

![MySQL 进阶：覆盖索引](assets/进阶-覆盖索引-04.png)

虽然是根据name字段查询，查询二级索引，但是由于查询返回在字段为 id，name，在name的二级索引中，这两个值都是可以直接获取到的，因为覆盖索引，所以不需要回表查询，性能高。

D. 执行SQL：selet id,name,gender from tb_user where name = 'Arm';

![MySQL 进阶：覆盖索引](assets/进阶-覆盖索引-05.png)
...
![MySQL 进阶：前缀索引](assets/进阶-前缀索引-01.png)
...
![MySQL 进阶：前缀索引](assets/进阶-前缀索引-02.png)
...
![MySQL 进阶：单列索引与联合索引](assets/进阶-单列索引与联合索引-01.png)

在查询出来的索引中，既有单列索引，又有联合索引。

接下来，我们来执行一条SQL语句，看看其执行计划：

![MySQL 进阶：单列索引与联合索引](assets/进阶-单列索引与联合索引-02.png)
...
![MySQL 进阶：单列索引与联合索引](assets/进阶-单列索引与联合索引-03.png)
...
![MySQL 进阶：单列索引与联合索引](assets/进阶-单列索引与联合索引-04.png)
...
![MySQL 进阶：大批量插入数据](assets/进阶-大批量插入数据-01.png)
...
![MySQL 进阶：大批量插入数据](assets/进阶-大批量插入数据-02.png)
...
![MySQL 进阶：主键优化](assets/进阶-主键优化-01.png)

行数据，都是存储在聚集索引的叶子节点上的。而我们之前也讲解过InnoDB的逻辑结构图：

![MySQL 进阶：主键优化](assets/进阶-主键优化-02.png)
...
![MySQL 进阶：主键优化](assets/进阶-主键优化-03.png)

②. 第一个页没有满，继续往第一页插入

![MySQL 进阶：主键优化](assets/进阶-主键优化-04.png)

③. 当第一个也写满之后，再写入第二个页，页与页之间会通过指针连接

![MySQL 进阶：主键优化](assets/进阶-主键优化-05.png)

④. 当第二页写满了，再往第三页写入

![MySQL 进阶：主键优化](assets/进阶-主键优化-06.png)

B. 主键乱序插入效果

①. 加入1#,2#页都已经写满了，存放了如图所示的数据

![MySQL 进阶：主键优化](assets/进阶-主键优化-07.png)

②. 此时再插入id为50的记录，我们来看看会发生什么现象

会再次开启一个页，写入新的页中吗？

![MySQL 进阶：主键优化](assets/进阶-主键优化-08.png)

不会。因为，索引结构的叶子节点是有顺序的。按照顺序，应该存储在47之后。

![MySQL 进阶：主键优化](assets/进阶-主键优化-09.png)

但是47所在的1#页，已经写满了，存储不了50对应的数据了。那么此时会开辟一个新的页 3#。

![MySQL 进阶：主键优化](assets/进阶-主键优化-10.png)

但是并不会直接将50存入3#页，而是会将1#页后一半的数据，移动到3#页，然后在3#页，插入50。

![MySQL 进阶：主键优化](assets/进阶-主键优化-11.png)

![MySQL 进阶：主键优化](assets/进阶-主键优化-12.png)

移动数据，并插入id为50的数据之后，那么此时，这三个页之间的数据顺序是有问题的。 1#的下一个页，应该是3#， 3#的下一个页是2#。所以，此时，需要重新设置链表指针。

![MySQL 进阶：主键优化](assets/进阶-主键优化-13.png)
...
![MySQL 进阶：主键优化](assets/进阶-主键优化-14.png)

当我们对已有数据进行删除时，具体的效果如下:

当删除一行记录时，实际上记录并没有被物理删除，只是记录被标记（flaged）为删除并且它的空间变得允许被其他记录声明使用。

![MySQL 进阶：主键优化](assets/进阶-主键优化-15.png)

当我们继续删除2#的数据记录

![MySQL 进阶：主键优化](assets/进阶-主键优化-16.png)

当页中删除的记录达到 MERGE_THRESHOLD（默认为页的50%），InnoDB会开始寻找最靠近的页（前或后）看看是否可以将两个页合并以优化空间使用。

![MySQL 进阶：主键优化](assets/进阶-主键优化-17.png)

![MySQL 进阶：主键优化](assets/进阶-主键优化-18.png)

删除数据，并将页合并之后，再次插入新的数据21，则直接插入3#页

![MySQL 进阶：主键优化](assets/进阶-主键优化-19.png)
...
![MySQL 进阶：主键优化](assets/进阶-主键优化-20.png)

![MySQL 进阶：主键优化](assets/进阶-主键优化-21.png)
...
![MySQL 进阶：orderby优化](assets/进阶-orderby优化-01.png)
...
![MySQL 进阶：orderby优化](assets/进阶-orderby优化-02.png)

```sql
explain select id,age,phone from tb_user order by age, phone ;
```

![MySQL 进阶：orderby优化](assets/进阶-orderby优化-03.png)
...
![MySQL 进阶：orderby优化](assets/进阶-orderby优化-04.png)

```sql
explain select id,age,phone from tb_user order by age , phone;
```

![MySQL 进阶：orderby优化](assets/进阶-orderby优化-05.png)
...
![MySQL 进阶：orderby优化](assets/进阶-orderby优化-06.png)
...
![MySQL 进阶：orderby优化](assets/进阶-orderby优化-07.png)
...
![MySQL 进阶：orderby优化](assets/进阶-orderby优化-08.png)

因为创建索引时，如果未指定顺序，默认都是按照升序排序的，而查询时，一个升序，一个降序，此时就会出现Using filesort。

![MySQL 进阶：orderby优化](assets/进阶-orderby优化-09.png)
...
![MySQL 进阶：orderby优化](assets/进阶-orderby优化-10.png)
...
![MySQL 进阶：orderby优化](assets/进阶-orderby优化-11.png)

升序/降序联合索引结构图示:

![MySQL 进阶：orderby优化](assets/进阶-orderby优化-12.png)

![MySQL 进阶：orderby优化](assets/进阶-orderby优化-13.png)
...
![MySQL 进阶：groupby优化](assets/进阶-groupby优化-01.png)
...
![MySQL 进阶：groupby优化](assets/进阶-groupby优化-02.png)
...
![MySQL 进阶：groupby优化](assets/进阶-groupby优化-03.png)

再执行如下的分组查询SQL，查看执行计划：

![MySQL 进阶：groupby优化](assets/进阶-groupby优化-04.png)

![MySQL 进阶：groupby优化](assets/进阶-groupby优化-05.png)
...
![MySQL 进阶：limit优化](assets/进阶-limit优化-01.png)
...
count(主 InnoDB 引擎会遍历整张表，把每一行的主键id 值都取出来，返回给服务层。
...
![MySQL 进阶：语法](assets/进阶-语法-01.png)
...
![MySQL 进阶：检查选项](assets/进阶-检查选项-01.png)
...
![MySQL 进阶：检查选项](assets/进阶-检查选项-02.png)
...
![MySQL 进阶：介绍](assets/进阶-介绍-01-2.png)
...
![MySQL 进阶：游标](assets/进阶-游标-01.png)

但是此时，tb_user_pro表结构及其数据都已经插入成功了，我们可以直接刷新表结构，检查表结构中的数据。

![MySQL 进阶：游标](assets/进阶-游标-02.png)
...
![MySQL 进阶：介绍|742](assets/进阶-介绍-01-3.png)
...
![MySQL 进阶：介绍](assets/进阶-介绍-02.png)
...
![MySQL 进阶：表锁](assets/进阶-表锁-01.png)

左侧为客户端一，对指定表加了读锁，不会影响右侧客户端二的读，但是会阻塞右侧客户端的写。

测试:

![MySQL 进阶：表锁](assets/进阶-表锁-02.png)

B. 写锁

![MySQL 进阶：表锁](assets/进阶-表锁-03.png)

左侧为客户端一，对指定表加了写锁，会阻塞右侧客户端的读和写。

测试:

![MySQL 进阶：表锁](assets/进阶-表锁-04.png)
...
![MySQL 进阶：元数据锁](assets/进阶-元数据锁-01.png)

当执行SELECT语句时，添加的是元数据共享锁（SHARED_READ），会阻塞元数据排他锁（EXCLUSIVE），之间是互斥的。

![MySQL 进阶：元数据锁](assets/进阶-元数据锁-02.png)
...
![MySQL 进阶：元数据锁](assets/进阶-元数据锁-03.png)
...
![MySQL 进阶：意向锁](assets/进阶-意向锁-01.png)

当客户端二，想对这张表加表锁时，会检查当前表是否有对应的行锁，如果没有，则添加表锁，此时就会从第一行数据，检查到最后一行数据，效率较低。

![MySQL 进阶：意向锁](assets/进阶-意向锁-02.png)

有了意向锁之后:

客户端一，在执行DML操作时，会对涉及的行加行锁，同时也会对该表加上意向锁。

![MySQL 进阶：意向锁](assets/进阶-意向锁-03.png)

而其他客户端，在对这张表加表锁的时候，会根据该表上所加的意向锁来判定是否可以成功加表锁，而不用逐行判断行锁情况了。

![MySQL 进阶：意向锁](assets/进阶-意向锁-04.png)
...
![MySQL 进阶：意向锁](assets/进阶-意向锁-05.png)

B. 意向排他锁与表读锁、写锁都是互斥的

![MySQL 进阶：意向锁](assets/进阶-意向锁-06.png)
...
![MySQL 进阶：介绍](assets/进阶-介绍-01-4.png)

间隙锁（Gap Lock）：锁定索引记录间隙（不含该记录），确保索引记录间隙不变，防止其他事务在这个间隙进行insert，产生幻读。在RR隔离级别下都支持。

![MySQL 进阶：介绍](assets/进阶-介绍-02-2.png)

临键锁（Next-Key Lock）：行锁和间隙锁组合，同时锁住数据，并锁住数据前面的间隙Gap。

在RR隔离级别下支持。

![MySQL 进阶：介绍](assets/进阶-介绍-03.png)
...
![MySQL 进阶：行锁](assets/进阶-行锁-01.png)
...
![MySQL 进阶：行锁](assets/进阶-行锁-02.png)

B. select...lock in share mode，加共享锁，共享锁与共享锁之间兼容。

![MySQL 进阶：行锁](assets/进阶-行锁-03.png)

共享锁与排他锁之间互斥。

![MySQL 进阶：行锁](assets/进阶-行锁-04.png)

客户端一获取的是id为1这行的共享锁，客户端二是可以获取id为3这行的排它锁的，因为不是同一行数据。而如果客户端二想获取id为1这行的排他锁，会处于阻塞状态，以为共享锁与排他锁之间互斥。

C. 排它锁与排他锁之间互斥

![MySQL 进阶：行锁](assets/进阶-行锁-05.png)
...
![MySQL 进阶：行锁](assets/进阶-行锁-06.png)

我们在两个客户端中执行如下操作:

![MySQL 进阶：行锁](assets/进阶-行锁-07.png)
...
![MySQL 进阶：行锁](assets/进阶-行锁-08.png)
...
![MySQL 进阶：间隙锁-临键锁](assets/进阶-间隙锁-临键锁-01.png)
...
![MySQL 进阶：间隙锁-临键锁](assets/进阶-间隙锁-临键锁-02.png)

![MySQL 进阶：间隙锁-临键锁](assets/进阶-间隙锁-临键锁-03.png)

C. 索引上的范围查询(唯一索引)--会访问到不满足条件的第一个值为止。

![MySQL 进阶：间隙锁-临键锁](assets/进阶-间隙锁-临键锁-04.png)
...
![MySQL 进阶：逻辑存储结构](assets/进阶-逻辑存储结构-01.png)
...
![MySQL 进阶：概述](assets/进阶-概述-01.png)

#### 6.2.2 内存结构

![MySQL 进阶：内存结构](assets/进阶-内存结构-01.png)
...
![MySQL 进阶：内存结构](assets/进阶-内存结构-02.png)
...
![MySQL 进阶：内存结构](assets/进阶-内存结构-03.png)
...
![MySQL 进阶：内存结构](assets/进阶-内存结构-04.png)

#### 6.2.3 磁盘结构

接下来，再来看看InnoDB体系结构的右边部分，也就是磁盘结构：

![MySQL 进阶：磁盘结构](assets/进阶-磁盘结构-01.png)
...
![MySQL 进阶：磁盘结构](assets/进阶-磁盘结构-02.png)
...
![MySQL 进阶：磁盘结构](assets/进阶-磁盘结构-03.png)

那也就是说，我们每创建一个表，都会产生一个表空间文件，如图：

![MySQL 进阶：磁盘结构](assets/进阶-磁盘结构-04.png)
...
![MySQL 进阶：磁盘结构](assets/进阶-磁盘结构-05.png)
...
![MySQL 进阶：磁盘结构](assets/进阶-磁盘结构-06.png)

前面我们介绍了InnoDB的内存结构，以及磁盘结构，那么内存中我们所更新的数据，又是如何到磁盘中的呢？此时，就涉及到一组后台线程，接下来，就来介绍一些InnoDB中涉及到的后台线程。

![MySQL 进阶：磁盘结构](assets/进阶-磁盘结构-07.png)

#### 6.2.4 后台线程

![MySQL 进阶：后台线程](assets/进阶-后台线程-01.png)
...
| 我们可以通过以下的这条指令，查看到 | InnoDB的状态信息，其中就包含IO Thread信息。 |   |

```sql
show engine innodb status \G;
```

![MySQL 进阶：后台线程](assets/进阶-后台线程-02.png)
...
![MySQL 进阶：redolog](assets/进阶-redolog-01.png)

那么，如何解决上述的问题呢？在InnoDB中提供了一份日志 redo log，接下来我们再来分析一下，通过redolog如何解决这个问题。

![MySQL 进阶：redolog](assets/进阶-redolog-02.png)

有了redolog之后，当对缓冲区的数据进行增删改之后，会首先将操作的数据页的变化，记录在redo log buffer中。在事务提交时，会将redo log buffer中的数据刷新到redo log磁盘文件中。

过一段时间之后，如果刷新缓冲区的脏页到磁盘时，发生错误，此时就可以借助于redo log进行数据恢复，这样就保证了事务的持久性。而如果脏页成功刷新到磁盘或或者涉及到的数据已经落盘，此时redolog就没有作用了，就可以删除了，所以存在的两个redolog文件是循环写的。
...
![MySQL 进阶：基本概念](assets/进阶-基本概念-01.png)
...
![MySQL 进阶：基本概念](assets/进阶-基本概念-02.png)
...
![MySQL 进阶：介绍](assets/进阶-介绍-01-5.png)
...
![MySQL 进阶：测试](assets/进阶-测试-01.png)

![MySQL 进阶：测试](assets/进阶-测试-02.png)
...
![MySQL 进阶：测试](assets/进阶-测试-04.png)

![MySQL 进阶：测试](assets/进阶-测试-05.png)

![MySQL 进阶：测试](assets/进阶-测试-05.png)

![MySQL 进阶：测试](assets/进阶-测试-07.png)
...
![MySQL 进阶：版本链](assets/进阶-版本链-01.png)
...
![MySQL 进阶：版本链](assets/进阶-版本链-02.png)

当事务2执行第一条修改语句时，会记录undo log日志，记录数据变更之前的样子; 然后更新记录，并且记录本次操作的事务ID，回滚指针，回滚指针用来指定如果发生回滚，回滚到哪一个版本。

![MySQL 进阶：版本链](assets/进阶-版本链-03.png)

B.第二步

![MySQL 进阶：版本链](assets/进阶-版本链-04.png)

当事务3执行第一条修改语句时，也会记录undo log日志，记录数据变更之前的样子; 然后更新记录，并且记录本次操作的事务ID，回滚指针，回滚指针用来指定如果发生回滚，回滚到哪一个版本。

![MySQL 进阶：版本链](assets/进阶-版本链-05.png)

C. 第三步

![MySQL 进阶：版本链](assets/进阶-版本链-06.png)

当事务4执行第一条修改语句时，也会记录undo log日志，记录数据变更之前的样子; 然后更新记录，并且记录本次操作的事务ID，回滚指针，回滚指针用来指定如果发生回滚，回滚到哪一个版本。

![MySQL 进阶：版本链](assets/进阶-版本链-07.png)
...
![MySQL 进阶：RC隔离级别](assets/进阶-RC隔离级别-01.png)

那么这两次快照读在获取数据时，就需要根据所生成的ReadView以及ReadView的版本链访问规则，到undolog版本链中匹配数据，最终决定此次快照读返回的数据。

A. 先来看第一次快照读具体的读取过程：

![MySQL 进阶：RC隔离级别](assets/进阶-RC隔离级别-02.png)

![MySQL 进阶：RC隔离级别](assets/进阶-RC隔离级别-03.png)

在进行匹配时，会从undo log的版本链，从上到下进行挨个匹配：

![MySQL 进阶：RC隔离级别](assets/进阶-RC隔离级别-04.png)

| 先匹配 这条记录，这条记录对应的 |
| --- |
| trx_id为4，也就是将4带入右侧的匹配规则中。 ①不满足 ②不满足 ③不满足 ④也不满足，都不满足，则继续匹配undo log版本链的下一条。再匹配第二条 ，这条记录对应的trx_id为3，也就是将3带入右侧的匹配规则中。①不满足 ②不满足 ③不满足 ④也不满足，都不满足，则继续匹配undo log版本链的下一条。 |

再匹配第三条 ，这条记录对应的trx_id为2，也就是将2带入右侧的匹配规则中。①不满足 ②满足终止匹配，此次快照读，返回的数据就是版本链中记录的这条数据。

B. 再来看第二次快照读具体的读取过程:

![MySQL 进阶：RC隔离级别](assets/进阶-RC隔离级别-05.png)

![MySQL 进阶：RC隔离级别](assets/进阶-RC隔离级别-06.png)
...
| 先匹配 这条记录，这条记录对应的 |
| --- |
| trx_id为4，也就是将4带入右侧的匹配规则中。 ①不满足 ②不满足 ③不满足 ④也不满足，都不满足，则继续匹配undo log版本链的下一条。再匹配第二条 ，这条记录对应的trx_id为3，也就是将3带入右侧的匹配规则中。①不满足 ②满足。终止匹配，此次快照读，返回的数据就是版本链中记录的这条数据。 |
...
![MySQL 进阶：RR隔离级别](assets/进阶-RR隔离级别-01.png)
...
![MySQL 进阶：RR隔离级别](assets/进阶-RR隔离级别-02.png)
...
![MySQL 进阶：mysql](assets/进阶-mysql-01.png)
...
![MySQL 进阶：mysqladmin](assets/进阶-mysqladmin-01.png)
...
![MySQL 进阶：mysqladmin](assets/进阶-mysqladmin-02.png)
...
![MySQL 进阶：mysqlbinlog](assets/进阶-mysqlbinlog-01.png)

上述查看到的二进制日志文件数据信息量太多了，不方便查询。我们可以加上一个参数 -s 来显示简单格式。

![MySQL 进阶：mysqlbinlog](assets/进阶-mysqlbinlog-02.png)
...
![MySQL 进阶：mysqlshow](assets/进阶-mysqlshow-01.png)

B. 查看数据库db01的统计信息

mysqlshow -uroot -p1234 db01 --count

![MySQL 进阶：mysqlshow](assets/进阶-mysqlshow-02.png)

C. 查看数据库db01中的course表的信息

mysqlshow -uroot -p1234 db01 course --count

![MySQL 进阶：mysqlshow](assets/进阶-mysqlshow-03.png)

D. 查看数据库db01中的course表的id字段的信息

mysqlshow -uroot -p1234 db01 course id --count

![MySQL 进阶：mysqlshow](assets/进阶-mysqlshow-04.png)
...
![MySQL 进阶：mysqldump](assets/进阶-mysqldump-01.png)

可以直接打开db01.sql，来查看备份出来的数据到底什么样。

![MySQL 进阶：mysqldump](assets/进阶-mysqldump-02.png)
...
![MySQL 进阶：mysqldump](assets/进阶-mysqldump-03.png)

打开 db02.sql ，来查看备份的数据，只有insert语句，没有备份表结构。

![MySQL 进阶：mysqldump](assets/进阶-mysqldump-04.png)
...
![MySQL 进阶：mysqldump](assets/进阶-mysqldump-05.png)

![MySQL 进阶：mysqldump](assets/进阶-mysqldump-06.png)

上述的两个文件 score.sql 中记录的就是表结构文件，而 score.txt 就是表数据文件，但是需要注意表数据文件，并不是记录一条条的insert语句，而是按照一定的格式记录表结构中的数据。如下：

![MySQL 进阶：mysqldump](assets/进阶-mysqldump-07.png)
...
---
[上一步：01.MySQL基础](01.MySQL基础.md) [下一步：03.MySQL运维](03.MySQL运维.md) [返回总索引](关于MySQL.md)
