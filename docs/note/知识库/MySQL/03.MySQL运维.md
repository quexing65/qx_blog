...
> [!warning] 历史环境与生产安全
> 本篇包含 MySQL 8.0.26、MyCat 1.6.7.3 及旧式主从术语的课程示例，适合学习原理和实验环境复现。生产部署前必须按当前 MySQL/MyCat 文档复核命令、复制术语、认证与网络安全；不要沿用示例弱密码或开放式远程授权。
...
![MySQL 运维：错误日志](assets/运维-错误日志-01.png)
...
![MySQL 运维：介绍](assets/运维-介绍-01.png)

![MySQL 运维：介绍](assets/运维-介绍-02.png)
...
![MySQL 运维：格式](assets/运维-格式-01.png)
...
![MySQL 运维：查询日志](assets/运维-查询日志-01.png)
...
![MySQL 运维：概述](assets/运维-概述-01.png)
...
![MySQL 运维：原理](assets/运维-原理-01.png)
...
![MySQL 运维：准备](assets/运维-准备-01.png)
...
![MySQL 运维：主库配置](assets/运维-主库配置-01.png)
...
![MySQL 运维：从库配置](assets/运维-从库配置-01.png)
...
![MySQL 运维：问题分析](assets/运维-问题分析-01.png)
...
![MySQL 运维：问题分析](assets/运维-问题分析-02.png)
...
![MySQL 运维：拆分策略](assets/运维-拆分策略-01.png)

#### 3.1.3 垂直拆分

1. 垂直分库

![MySQL 运维：垂直拆分](assets/运维-垂直拆分-01.png)
...
![MySQL 运维：垂直拆分](assets/运维-垂直拆分-02.png)
...
![MySQL 运维：水平拆分](assets/运维-水平拆分-01.png)
...
![MySQL 运维：水平拆分](assets/运维-水平拆分-02.png)
...
![MySQL 运维：实现技术](assets/运维-实现技术-01.png)
...
![MySQL 运维：介绍](assets/运维-介绍-01-2.png)
...
![MySQL 运维：下载](assets/运维-下载-01.png)
...
![MySQL 运维：目录介绍](assets/运维-目录介绍-01.png)
...
![MySQL 运维：概念介绍](assets/运维-概念介绍-01.png)
...
![MySQL 运维：需求](assets/运维-需求-01.png)
...
![MySQL 运维：环境准备](assets/运维-环境准备-01.png)
...
![MySQL 运维：启动](assets/运维-启动-01.png)
...
![MySQL 运维：schema.xml](assets/运维-schema.xml-01.png)
...
![MySQL 运维：schema标签](assets/运维-schema标签-01.png)
...
![MySQL 运维：schema标签](assets/运维-schema标签-02.png)
...
![MySQL 运维：datanode标签](assets/运维-datanode标签-01.png)
...
![MySQL 运维：datahost标签](assets/运维-datahost标签-01.png)

该标签在MyCat逻辑库中作为底层标签存在, 直接定义了具体的数据库实例、读写分离、心跳语句。

核心属性：

name：唯一标识，供上层标签使用maxCon/minCon：最大连接数/最小连接数
...
![MySQL 运维：rule.xml](assets/运维-rule.xml-01.png)
...
![MySQL 运维：server.xml](assets/运维-server.xml-01.png)
...
| useHandshakeV10 | 0,1 | 的jdbc驱动, 是否采用HandshakeV10Packet来与client进行通 |
|   |   | 信, 1:是, 0:否 |
...
| processors | 1,2.... | 影响processorBufferPool,processorBufferLocalPercent, |
|   |   | processorExecutor属性, 所有, 在性能 |
|   |   | 调优时, 可以适当地修改processors值 |
|   |   | 指定每次分配Socket Direct Buffer默认 |
| processorBufferChunk |   | 值为4096字节, 也会影响BufferPool长度,如果一次性获取字节过多而导致buffer不够 |
|   |   | 用, 则会出现警告, 可以调大该值 |
|   |   | 指定NIOProcessor上共享 |
...
![MySQL 运维：server.xml](assets/运维-server.xml-02.png)
...
![MySQL 运维：场景](assets/运维-场景-01.png)

现在考虑将其进行垂直分库操作，将商品相关的表拆分到一个数据库服务器，订单表拆分的一个数据库服务器，用户及省市区表拆分到一个服务器。最终结构如下：

![MySQL 运维：场景](assets/运维-场景-02.png)

##### 3.5.1.2 准备

准备三台服务器，IP地址如图所示：

![MySQL 运维：准备](assets/运维-准备-01-2.png)
...
![MySQL 运维：测试](assets/运维-测试-01.png)
...
![MySQL 运维：测试](assets/运维-测试-02.png)
...
![MySQL 运维：测试](assets/运维-测试-03.png)
...
![MySQL 运维：全局表](assets/运维-全局表-01.png)
...
![MySQL 运维：全局表](assets/运维-全局表-02.png)
...
![MySQL 运维：场景](assets/运维-场景-01-2.png)

##### 3.5.2.2 准备

准备三台服务器，具体的结构如下：

![MySQL 运维：准备](assets/运维-准备-01-3.png)
...
![MySQL 运维：范围分片](assets/运维-范围分片-01.png)
...
![MySQL 运维：取模分片](assets/运维-取模分片-01.png)
...
![MySQL 运维：一致性hash分片](assets/运维-一致性hash分片-01.png)
...
![MySQL 运维：枚举分片](assets/运维-枚举分片-01.png)
...
![MySQL 运维：应用指定算法](assets/运维-应用指定算法-01.png)
...
该算法类似于十进制的求模运算，但是为二进制的操作，例如，取 id 的二进制低 10 位与1111111111 进行位 & 运算，位与运算最小值为 0000000000，最大值为1111111111，转换为十进制，也就是位于0-1023之间。

![MySQL 运维：固定分片hash算法](assets/运维-固定分片hash算法-01.png)
...
![MySQL 运维：固定分片hash算法](assets/运维-固定分片hash算法-02.png)
...
![MySQL 运维：字符串hash解析算法](assets/运维-字符串hash解析算法-01.png)
...
![MySQL 运维：字符串hash解析算法](assets/运维-字符串hash解析算法-02.png)
...
![MySQL 运维：按天分片算法](assets/运维-按天分片算法-01.png)
...
<!--
    从开始时间开始，每10天为一个分片，到达结束时间之后，会重复开始分片插入
     配置表的 dataNode 的分片，必须和分片规则数量一致，例如 2022-01-01 到 2022-12-31 ，每 10天一个分片，一共需要37个分片。
 -->
```
...
![MySQL 运维：自然月分片](assets/运维-自然月分片-01.png)
...
<!--
    ...
    配置表的 dataNode 的分片，必须和分片规则数量一致，例如 2022-01-01 到 2022-12-31 ，一 共需要12个分片。
-->
```
...
![MySQL 运维：MyCat原理](assets/运维-MyCat原理-01.png)
...
![MySQL 运维：访问](assets/运维-访问-01.png)
...
![MySQL 运维：配置](assets/运维-配置-01.png)
...
![MySQL 运维：测试](assets/运维-测试-01-2.png)

B. 物理节点

![MySQL 运维：测试](assets/运维-测试-02-2.png)

C. SQL统计

![MySQL 运维：测试](assets/运维-测试-03-2.png)

D. SQL表分析

![MySQL 运维：测试](assets/运维-测试-04.png)

E. SQL监控

![MySQL 运维：测试](assets/运维-测试-05.png)

F. 高频SQL

![MySQL 运维：测试](assets/运维-测试-06.png)
...
![MySQL 运维：介绍](assets/运维-介绍-01-3.png)
...
![MySQL 运维：原理](assets/运维-原理-01-2.png)
...
![MySQL 运维：schema.xml配置](assets/运维-schema.xml配置-01.png)
...
![MySQL 运维：测试](assets/运维-测试-01-3.png)
...
![MySQL 运维：介绍](assets/运维-介绍-01-4.png)
...
![MySQL 运维：主库配置](assets/运维-主库配置-01-2.png)
...
![MySQL 运维：主库配置](assets/运维-主库配置-02.png)

2). Master2(192.168.200.213)

![MySQL 运维：主库配置](assets/运维-主库配置-03.png)
...
![MySQL 运维：主库配置](assets/运维-主库配置-04.png)

##### 4.4.3.2 从库配置

1). Slave1(192.168.200.212)

![MySQL 运维：从库配置](assets/运维-从库配置-01-2.png)
...
![MySQL 运维：从库配置](assets/运维-从库配置-02.png)
...
![MySQL 运维：从库关联主库](assets/运维-从库关联主库-01.png)
...
![MySQL 运维：从库关联主库](assets/运维-从库关联主库-02.png)

2). 两台主库相互复制

![MySQL 运维：从库关联主库](assets/运维-从库关联主库-03.png)
...
![MySQL 运维：从库关联主库](assets/运维-从库关联主库-04.png)
...
![MySQL 运维：配置](assets/运维-配置-01-2.png)
...
---
[上一步：02.MySQL进阶](02.MySQL进阶.md) [返回总索引](关于MySQL.md)
