...
def bubble_sort(my_list):  # 原地排序：函数会修改传入的列表对象.
    # 1.1 获取列表的长度.
...
* 代码实现
  ...
  def select_sort(my_list):   # 原地排序：函数会修改传入的列表对象.
      # 1. 获取列表长度.
      n = len(my_list)
      # 2. 外循环, 控制比较的: 轮数.
      for i in range(n - 1):
...
def insert_sort(my_list):   # 原地排序：函数会修改传入的列表对象.
    # 1. 获取列表长度.
...
def binary_search_recursion(my_list, target, start=0, end=None):
    """
    ...
    if my_list[mid] == target:
        ...
        return binary_search_recursion(my_list, target, start, mid - 1)
    else:
        return binary_search_recursion(my_list, target, mid + 1, end)
...
* 代码框架
  ...
  树结构解释:
      概述:
          树是连通且无环的层次结构；这里讨论的是有根树.
      特点:
          1. 有且只有 1 个根节点.
          2. 除根节点外，每个节点恰好有 1 个父节点；一个节点可以有 0 个或多个子节点.
          3. 没有子节点的节点, 称之为: 叶子节点.
      常用分类:
          无序树:
          有序树:
          二叉树:
              二叉树: 每个节点最多有 2 个子节点.
              完全二叉树: 除最后一层外各层都满，最后一层的节点从左到右连续排列.
              满二叉树（完美二叉树）: 每一层都被填满.
              平衡二叉树: 常见定义要求任意节点左右子树高度差不超过 1；不同平衡树还包含额外规则.
  
          我们用的最多的就是: 二叉树
      存储:
          顺序存储: 使用数组下标隐式表达父子关系，适合完全二叉树.
          链式存储: 节点显式保存 item、lchild、rchild 等引用，适合一般二叉树.
...
[上一步：09.数据结构](09.数据结构.md) [返回总索引](关于Python进阶.md)
