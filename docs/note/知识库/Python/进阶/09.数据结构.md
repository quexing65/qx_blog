...
* 算法
  ...
  > 就是解决问题的思路和方法。算法具有独立性，不依赖某一种编程语言；同一算法可以用 Python、Java 等语言实现.
  ...
  * 如何衡量算法的优劣
    ...
    ![大O标记法](assets/大O标记法.png)

  * 最优和最坏时间复杂度

    > 如非特殊说明, 我们考虑的都是 **最坏时间复杂度**, 因为它是算法的一种保证.
    >
    > 最好、平均和最坏复杂度分别描述不同输入下的表现。工程分析通常重点关注平均与最坏情况，而不是认为最好情况“没有价值”.
  ...
  * 常见的空间复杂度如下
    ...
    > 空间复杂度同样重要。内存限制、缓存局部性、数据复制和并发任务数量都可能决定程序能否稳定运行.
    >
    > **从最优到最坏分别是:** 
    >
...
* 分类
  ...
  * 线性结构
    ...
    > **特点:** 元素呈线性次序；除首尾外，每个元素通常有唯一直接前驱和直接后继.
    >
    > **例如:** 顺序表(栈, 队列), 链表

  * 非线性结构

    > **特点:** 元素关系不是单一线性次序。例如树中非根节点只有一个父节点，图中的节点则可能连接多个节点.
    >
    > **例如:** 树, 图 
...
* 图解
  ...
  ![顺序表组成](assets/顺序表组成.png)

* 顺序表存储方式详解

  * 一体式存储

    ![顺序表一体式存储](assets/顺序表一体式存储.png)

  * 分离式存储

    ![顺序表分离式存储](assets/顺序表分离式存储.png)
...
* 特点
  ...
  * 数据区和信息区分开存储的 -> 分离式存储（扩容时仍需复制元素引用，但保存长度、容量等信息的对象本身可以保持稳定）.
...
* 概述
  ...
  > 链表属于线性结构，各节点不要求连续存储。已知节点位置时，插入和删除只需调整链接；但按索引访问需要 `O(n)`，且额外指针和较差的缓存局部性会增加成本。
  > Python 内置 `list` 是动态数组，通常更通用；不要仅因“可能扩容”就改用手写链表，应根据访问与修改模式选择。
...
* 图解
  ...
  ![链表节点结构](assets/链表节点结构.png)

  ![链表分类](assets/链表分类.png)
...
[上一步：08.正则表达式](08.正则表达式.md) [下一步：10.排序查找与二叉树](10.排序查找与二叉树.md) [返回总索引](关于Python进阶.md)
