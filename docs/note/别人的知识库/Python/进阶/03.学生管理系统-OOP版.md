...
* 删除学生信息
  ...
  def del_student(self):
      ...
      for stu in self.stu_list:
          ...
          # for 循环正常结束且没有执行 break，说明没有找到该学生.
          print('查无此人, 请检查后重新删除!\n')
  ```
...
```python
# 使用 JSON 保存结构化数据，避免 eval 执行文件中的任意代码.
import json
...
def save_student(self):
    ...
    with open('./stu_data.txt', 'w', encoding='utf-8') as dest_f:
        ...
        # 9.3 把字典列表持久化为 JSON.
        json.dump(stu_dict, dest_f, ensure_ascii=False)
```
...
[上一步：02.面向对象进阶](02.面向对象进阶.md) [下一步：04.迭代器生成器与property](04.迭代器生成器与property.md) [返回总索引](关于Python进阶.md)
