...
> 从类、对象和 self 入门，通过魔法方法、案例与继承建立完整的面向对象基础。
...
魔法方法:
    ...
    常用的魔法方法:
        ...
        __str__()       当用print()函数 打印对象的时候, 会自动调用该对象(所在类)的 str魔法方法.
                        该魔法方法默认打印的是对象的地址值, 无意义, 一般都会重写, 改为打印 对象的各个属性值.
        __del__()       对象被垃圾回收时可能调用，但调用时机和解释器退出阶段的执行均不可靠.
                        del 删除的是名称绑定，不等于立刻销毁对象；不要依赖 __del__ 释放关键资源.
"""
...
# 7. 删除名称 c1；如果没有其他引用，CPython 通常会立即回收对象，但其他实现不保证时机.
# del c1
# print(c1)       # NameError：名称已删除.

print('程序结束!')
```

> 文件、网络连接和锁等资源应使用 `with` 上下文管理器或显式 `close()` 管理，不要把 `__del__` 当作可靠的资源清理机制。
...
[下一步：02.面向对象进阶](02.面向对象进阶.md) [返回总索引](关于Python进阶.md)
